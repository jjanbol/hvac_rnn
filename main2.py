...
# --- MODEL LOADING ---
# Create function to load the model - moved outside of cached function
def init_model():
    try:
        model = MLP(
...
# Use simpler caching mechanism to avoid Streamlit-PyTorch interaction issues
@st.cache_resource
def get_model():
    return init_model()
...
def get_model_prediction(df_yolo, df_people, df_pico):
    ...
    try:
        ...
        # Get the model - using the simpler caching method
        model = get_model()
        if model is None:
        ...
        # Create input tensor with all three values
        try:
            input_tensor = torch.tensor([float(depth_input), float(rgb_input), float(pico_input)], 
                                        dtype=torch.float32).unsqueeze(0).to(device)
        except (TypeError, ValueError) as e:
            return None, f"Error creating input tensor: {e}. Input values: depth={depth_input}, rgb={rgb_input}, pico={pico_input}", None
...
with prediction_tab:
    ...
    # Manual refresh button (removed duplicate)
    if st.button("🔄 Refresh Prediction", key="refresh_prediction"):
        st.experimental_rerun()
    
    # Manual refresh button (previously duplicated)
